...
class YaMoneyCommonHttpProtocol {
    ...
    public function processRequest($request) {
		...
		$this->log("Order #" . (int)$request['orderNumber'] );
		try {
			$this->order = \Sale\Order::getById( (int)$request['orderNumber'] );
			$this->gateway = $this->order->getPaymentGateway();
		} catch (\Exception $e) {
			$this->log("Error: " . $e->getMessage() );
            $response = $this->buildResponse($this->action, $request['invoiceId'], 1, $e->getMessage());
            $this->sendResponse($response);
		    return;			
		}
		
		$this->log("Security type " . $this->gateway->SECURITY_TYPE);
		
        if ($this->gateway->SECURITY_TYPE == "MD5") {
            $this->log("Request: " . print_r($request, true));
            // If the MD5 checking fails, respond with "1" error code
            if (!$this->checkMD5($request)) {
                $response = $this->buildResponse($this->action, $request['invoiceId'], 1, 'Checksum fail');
                $this->sendResponse($response);
				return;
            }
        } else if ($this->gateway->SECURITY_TYPE == "PKCS7") {
    ...
    private function checkOrder($request) {
        $response = null;
        if ($request['orderSumAmount'] != $this->order->getTotal()) {
            $response = $this->buildResponse($this->action, $request['invoiceId'], 100, "Incorrect amount");
        } else {
    ...
    private function paymentAviso($request) {
		$this->gateway->saveTransaction($request['invoiceId'], $request);	
    ...
    private function checkMD5($request) {
        $str = $request['action'] . ";" .
            ...
            $request['invoiceId'] . ";" . trim($request['customerNumber']) . ";" . $this->gateway->params['shop_password'];
    ...
    private function buildResponse($functionName, $invoiceId, $result_code, $message = null) {
        try {
            ...
            $response = '<?xml version="1.0" encoding="UTF-8"?><' . $functionName . 'Response performedDatetime="' . $performedDatetime .
                '" code="' . $result_code . '" ' . ($message != null ? 'message="' . $message . '"' : "") . ' invoiceId="' . $invoiceId . '" shopId="' . $this->gateway->params['shopId'] . '"/>';
            return $response;
        } catch (\Exception $e) {
            $this->log($e);
        }
    ...
    private function log($str) {
        ...
        $str = $str . "\n";
        file_put_contents(__DIR__.'/../../logs/log.txt', '[' . date("Y-m-d H:i:s") . '] ' . $str, FILE_APPEND);
    }
